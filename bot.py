...
FOOTBALL_LEAGUES = {
,"כדורגל - ליגת פרמייר" :"epl_soccer "
"soccer_spain_la_liga": "ליגה לה - כדורגל",
"soccer_germany_bundesliga": "בונדסליגה - כדורגל",
"soccer_italy_serie_a": "א סריה - כדורגל",
"soccer_uefa_champs_league": "האלופות ליגת - כדורגל",
"soccer_israel_premier_league": "העל ליגת - כדורגל",
}
BASKETBALL_LEAGUES = {
"basketball_nba": "כדורסל - NBA",
,"כדורסל - יורוליג" :"euroleague_basketball "
"basketball_israel_premier_league": "ליגת - כדורסל Winner",
}
VALUE_MIN = 1.55
VALUE_MAX = 2.15
...
return
(".הבוט פעיל. הדוח היומי נשלח אוטומטית בשעה 10:00" ,message(to_reply.bot
...
logger.warning(f"{sport_key} not active")
else:
logger.error(f"API {resp.status_code} {sport_key}")
except requests.RequestException as e:
logger.error(f"Network error {sport_key}: {e}")
return [], None
...
"home": home,
"away": away,
"recommendation": "נצחון " + team_name,
...
lines = ["\n<b>" + label + "</b>"]
if not top:
(".לא נמצאו הזדמנויות היום")append.lines
return lines, False
b = top[0]
lines += [
,"<b/<:הבאנקר היומי<b "<
"<b>" + b["home"] + " נגד " + b["away"] + "</b>",
"ליגה: " + b["league"],
"המלצה: " + b["recommendation"],
"יחס:> b>" + str(b["odd"]) + "</b>",
"שעה: " + b["date"] + " " + b["time"],
"סוכנויות: " + str(b["bookmaker_count"]) + " | תנודתיות: " + str(b["volatility"]),
"",
]
doubles = top[1:3]
...
lines += [
,"<b/<:הדאבל היומי<b "<
"משולב יחס:> b>" + str(combined) + "</b>",
"",
]
for i, d in enumerate(doubles, 1):
lines += [
"משחק " + str(i) + ": " + d["home"] + " נגד " + d["away"],
"ליגה: " + d["league"],
"המלצה: " + d["recommendation"] + " | יחס: " + str(d["odd"]) + " | שעה: " + d["date"] + " " + d["time"],
"",
]
else:
(".אין מספיק משחקים לדאבל היום")append.lines
return lines, True
def build_report():
football_raw = []
basketball_raw = []
...
football_lines, has_f = format_section(ft, "כדורגל("
basketball_lines, has_b = format_section(bt, "כדורסל("

lines += football_lines
lines.append("---")
lines += basketball_lines
if not has_f and not has_b:
remaining_last + " :יתרת שאילתותn\n\.היום לא נמצאו הזדמנויות בעלות ערך גבוה במערכתn\n>\b/<דוח יומי<b "<return
lines += [
"---",
,"<b/ "<+ remaining_last +>" b <:יתרת שאילתות לחודש זה"
...
try:
bot.send_message(MY_CHAT_ID, "הדוח בהפקת שגיאה: " + str(e))
except Exception:
pass
